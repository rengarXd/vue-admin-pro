...
var getUserMedia = mediaDevices ? mediaDevices.getUserMedia : undefined
...
MP3Recorder.prototype = {
  ...
  start: function (success, error) {
    ...
    var onFrameSplite = ops.framesplite
    var ondataBuffer = ops.dataBufferEvent
    navigator.getUserMedia({ audio: true, video: false }, function (stream) {
      ...
      if (MediaRecorder && ops.useMediaRecorder !== false) {
        ...
        recorder.addEventListener('stop', function () {
          ...
          var ext = blob ? (blob.type || '').match(/^[^\/]+\/([^;]+);?.*?$/)[1] || '' : ''
        ...
        worker.onmessage = function (e) {
          var obj = e.data; var data = obj.data
          ...
          switch (obj.cmd) {
            ...
            case 'frameData':
              // console.log(data)
              // if (onFrameSplite) onFrameSplite(buf2hex(data))
              if (onFrameSplite) onFrameSplite(data)
              break
            case 'dataBuffer':
              if (ondataBuffer) ondataBuffer(data)
            case 'error':
